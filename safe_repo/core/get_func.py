    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            if msg.media == MessageMediaType.VIDEO and msg.video.mime_type in ["video/mp4", "video/x-matroska"]:
                ...
                if duration <= 300:
                    short_caption = apply_caption_rules(sender, msg.caption if msg.caption else '')
                    custom_caption = get_user_caption_preference(sender)
                    if custom_caption:
                        short_caption = f"{short_caption}\n\n__**{custom_caption}**__" if short_caption else f"__**{custom_caption}**__"
                    safe_repo = await app.send_video(chat_id=sender, video=file, caption=short_caption or None, height=height, width=width, duration=duration, thumb=None, progress=progress_bar, progress_args=('**UPLOADING:**\n', edit, time.time())) 
                    if msg.pinned_message:
                        try:
                            await safe_repo.pin(both_sides=True)
                        except Exception as e:
...
def apply_caption_rules(user_id, text):
    """Apply remove-word and replacement-word rules to caption text."""
    text = text or ''
    for word in load_delete_words(user_id):
        text = text.replace(str(word), '')
    for word, replace_word in load_replacement_words(user_id).items():
        text = text.replace(str(word), str(replace_word))
    return text
    ...
    if user_id in sessions:
        ...
        if session_type == 'setchat':
            ...
            try:
                ...
                if not match:
                    await event.respond("Usage: 'WORD' 'REPLACEWORD'\nExample: 'rail' 'not'")
                else:
                    word = match.group(1).strip()
                    replace_word = match.group(2).strip()
                    if not word or not replace_word:
                        await event.respond("Both words must be non-empty.")
                    else:
                        ...
                        if word in delete_words:
                            ...
                            replacements[str(word)] = str(replace_word)
